...
export default function SharesManagementPage() {
  ...
  const handleDeactivateShare = async (shareToken: string) => {
    if (!confirm('Are you sure you want to deactivate this share? This will make the link inaccessible.')) {
      return;
    }
  ...
  const filteredShares = shares.filter(share => {
    ...
    const matchesFilter = filterActive === null || share.is_active === filterActive;
            ...
            <div className="text-center py-12">
              ...
              {!searchTerm && filterActive === null && (
                <p className="text-sm text-gray-400">
                  Visit any folder page and click the "Share" button to create your first share.
                </p>
              )}
            </div>
          ) : (
                    ...
                    <tr key={share.id} className="hover:bg-gray-50">
                        ...
                        <div className="flex items-start">
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-medium text-gray-900">
                              {share.folder_name || share.folder_path.split('/').pop()}
                            </p>
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          <div className="flex items-center text-sm">
                            {share.has_password ? (
                              <>
                                ...
                                <span className="text-green-700">Password protected</span>
                              </>
                            ) : (
                              <>
                          ...
                          <div className="flex items-center text-sm">
                            ...
                            <span className={share.allow_download ? "text-blue-600" : "text-gray-500"}>
                              {share.allow_download ? "Downloads enabled" : "View only"}
                            </span>
                          </div>
                          {share.expires_at && (
                            <div className="flex items-center text-sm">
                              <Calendar className="w-3 h-3 mr-1" />
                              <span className={isExpired(share.expires_at) ? "text-red-600" : "text-orange-600"}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => window.open(`/share/${share.share_token}`, '_blank')}
                            className="p-1 text-gray-400 hover:text-blue-600 transition-colors"
                            title="Open share"
                          >
                          ...
                          <button
                            onClick={() => handleDeactivateShare(share.share_token)}
