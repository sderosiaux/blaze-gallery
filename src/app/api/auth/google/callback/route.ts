...
import { validateGoogleUser, validateState, authRateLimiter } from '@/lib/auth/validators';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!authConfig.enabled) {
      return NextResponse.redirect(new URL('/?error=auth_disabled', request.url));
    }
    ...
    if (!storedState || !storedCodeVerifier) {
      return NextResponse.redirect(new URL('/?error=missing_oauth_data', request.url));
    }
    ...
    try {
      ...
      console.warn('Failed to get user info from API, using ID token info:', error);
    ...
    if (!validation.valid) {
      ...
      return NextResponse.redirect(new URL(`/?error=unauthorized&reason=${encodeURIComponent(validation.errors[0])}`, request.url));
    }
