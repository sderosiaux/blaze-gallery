...
class BlazeGalleryMCPHandler {
  async searchPhotos(filters: SearchFilters): Promise<Photo[]> {
    ...
    if (filters.folder_path) {
      conditions.push(`(f.path = $${paramCount} OR f.path LIKE $${paramCount + 1})`);
  ...
  async getPhotoAnalytics(options: {
    ...
    const queryText = options.groupBy === 'folder' ? `
      SELECT
        ${selectFields},
  ...
  async getPhotoTrends(options: {
    ...
    switch (timeRange) {
      case 'last-30-days':
        dateFilter = "AND modified_at >= CURRENT_TIMESTAMP - INTERVAL '30 days'";
        break;
      case 'last-year':
...
const toolSchemas = {
  search_photos: {
    inputSchema: {
      ...
      properties: {
        ...
        has_metadata: { type: 'boolean', description: 'Filter by metadata presence' },
  ...
  get_photo: {
    inputSchema: {
      type: 'object',
      properties: {
        photo_id: { type: 'number', description: 'The ID of the photo to retrieve' },
      },
      required: ['photo_id'],
    },
    outputSchema: { type: 'object' },
  ...
  search_folders: {
    inputSchema: {
      type: 'object',
      properties: {
        parent_path: { type: 'string', description: 'Filter by parent folder path' },
        folder_name: { type: 'string', description: 'Filter by folder name (partial match)' },
        has_photos: { type: 'boolean', description: 'Filter by photo presence' },
  ...
  get_folder: {
    inputSchema: {
      type: 'object',
      properties: {
        folder_path: { type: 'string', description: 'The path of the folder to retrieve' },
      },
  ...
  get_folder_tree: {
    inputSchema: {
      type: 'object',
      properties: {
        root_path: { type: 'string', description: 'Root path for the tree (optional)' },
      },
    },
    outputSchema: { type: 'object' },
  },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        orderBy: {
          ...
          description: 'Order results by period, count, or size (default: period)',
        },
        orderDirection: {
          type: 'string',
          enum: ['ASC', 'DESC'],
        ...
        return NextResponse.json({
          tools: [
            ...
            },
            {
              ...
              description: 'Get efficient analytics breakdown of photos by year, month, or folder',
          ...
          switch (name) {
            ...
            case 'get_photo': {
              if (!args || typeof args.photo_id !== 'number') {
                return NextResponse.json(
                  { error: { code: ErrorCode.InvalidRequest, message: 'photo_id is required and must be a number' } },
                  { status: 400 }
                );
              }
              ...
              if (!photo) {
                return NextResponse.json(
                  { error: { code: ErrorCode.InvalidRequest, message: `Photo with ID ${args.photo_id} not found` } },
                  { status: 404 }
                );
              }
              return NextResponse.json({
            ...
            case 'get_folder': {
              if (!args || typeof args.folder_path !== 'string') {
                return NextResponse.json(
                  { error: { code: ErrorCode.InvalidRequest, message: 'folder_path is required and must be a string' } },
                  { status: 400 }
                );
              }
              ...
              if (!folder) {
                return NextResponse.json(
                  { error: { code: ErrorCode.InvalidRequest, message: `Folder with path "${args.folder_path}" not found` } },
                  { status: 404 }
                );
              }
              return NextResponse.json({
            ...
            case 'get_photo_analytics': {
              if (!args || typeof args.groupBy !== 'string') {
                return NextResponse.json(
                  { error: { code: ErrorCode.InvalidRequest, message: 'groupBy is required and must be a string' } },
                  { status: 400 }
                );
              }
            ...
            case 'get_photo_trends': {
              const options = {
                timeRange: (args && args.timeRange as 'last-30-days' | 'last-year' | 'all-time') || 'last-year',
                groupBy: (args && args.groupBy as 'day' | 'week' | 'month') || 'month',
                metric: (args && args.metric as 'count' | 'size' | 'favorites') || 'count',
              };
            ...
            default:
              return NextResponse.json(
                { error: { code: ErrorCode.MethodNotFound, message: `Unknown tool: ${name}` } },
                { status: 404 }
              );
          }
        } catch (error) {
