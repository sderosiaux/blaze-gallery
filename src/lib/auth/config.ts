...
function getAuthConfig(): AuthConfig {
  ...
  return {
    ...
    emailWhitelist: process.env.AUTH_EMAIL_WHITELIST
      ? process.env.AUTH_EMAIL_WHITELIST.split(',').map(email => email.trim().toLowerCase())
      : [],
...
export function validateAuthConfig(): { valid: boolean; errors: string[] } {
  ...
  if (!config.googleClientSecret) {
    errors.push('GOOGLE_CLIENT_SECRET is required when authentication is enabled');
  }

  if (!config.sessionSecret || config.sessionSecret.length < 32) {
    errors.push('AUTH_SESSION_SECRET must be at least 32 characters when authentication is enabled');
  }

  if (config.emailWhitelist.length === 0) {
    errors.push('AUTH_EMAIL_WHITELIST cannot be empty when authentication is enabled');
  }

  // Validate redirect URI format
  if (config.googleRedirectUri && !config.googleRedirectUri.startsWith('http')) {
